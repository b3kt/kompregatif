...
class OptionsController extends BaseVoyagerUserController
{
    /**
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if(isset($options->filter)){
                    $filter = app($options->filter->model)
                        ->where($options->filter->field, $options->filter->value)
                        ->first();
                    $model = $model->where($options->filter->filter_on, $filter->{$options->filter->filter_by});
                }
